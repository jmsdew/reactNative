...
// 
const database = SQLite.openDatabase("places.db");  // 기존에 db가 있으면 커넥션만 맺어서 가져옴
    ...
    database.transaction((tr) =>{
      ...
      (_,result) => {
        ...
        for(const dp of result.rows._array){
          place.push(new Places(dp.title, dp.imageUri, {lat: dp.lat, ing:dp.lng, address:dp.address}, dp.id));
        } 
        resolve(place);
      },
